...
static bool write(struct i2c_bus * handle, uint8_t data) {
    ...
    while (I2C2STAT & I2C_STAT_TRSTAT);

    if (I2C2STAT & (I2C_STAT_ACKSTAT | I2C_STAT_BCL)) {
...
static uint8_t read(struct i2c_bus * handle) {
    ...
    while ((I2C2CON & (I2C_CON_SEN | I2C_CON_RSEN | I2C_CON_PEN | I2C_CON_RCEN | I2C_CON_ACKEN)) != 0);
    I2C2CON |= I2C_CON_RCEN;

    while ((I2C2STAT & I2C_STAT_RBF) == 0);

    while (I2C2CON & I2C_CON_RCEN);
...
static void ack(struct i2c_bus * handle) {
    ...
    I2C2CONCLR = I2C_CON_ACKDT;
    I2C2CONSET = I2C_CON_ACKEN;
}
