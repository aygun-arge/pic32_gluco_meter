...
void init_voc_draw(void)
{
    char buff[64];

    drawFill(COLOR_BLUE);

    drawString(65, 15, COLOR_WHITE, &verdanabold14ptFontInfo, "VOC type");

    snprintf(buff, sizeof(buff), "Choose VOC type:");
    drawString(18, 76, COLOR_WHITE, &verdanabold14ptFontInfo, buff);

    drawButton(60, 200, 120, 40, &dejaVuSansBold9ptFontInfo, 7, COLOR_BLACK, COLOR_RED, COLOR_WHITE, "Higher range");
    drawButton(60, 260, 120, 40, &dejaVuSansBold9ptFontInfo, 7, COLOR_BLACK, COLOR_RED, COLOR_WHITE, "Lower range");
    gui_set_update(init_voc_events);
}
